...
public class Wallis_Product {
	...
	public static Double ValRec(double p_Num, boolean p_switch, double p_top, double p_bot) // Nur ein bisschen rumgespielt, höchstwahrscheinlich eher mäßig effektiv implementiert.
	{
		if(p_Num > 0)
		{
			...
			/*
			 * 
			 * 	Alte Variante, oben so weit optimiert, dass er deutlich höhere Zahlen handlen kann.	(bis zu 6000 anstatt 3000!!! Hab noch überlegt mit float anstatt double
			 * zu arbeiten da es nur 4 bytes zieht, aber das würde dann zu sehr auf die Genauigkeit gehen.
			boolean l_switch = p_switch;
			double l_top = p_top;
			double l_bot = p_bot;
